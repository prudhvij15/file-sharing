...
//             <FaTrash
//               className="cursor-pointer text-red-500 text-xs mt-1 m-a"
    ...
    <div className="flex flex-wrap justify-start">
      {files.map((file) => (
        <div
          key={file._id}
          className="bg-gray-200 rounded-md p-1 flex flex-col items-center mr-2 mb-2"
          style={{ width: "calc(20% - 0.5rem)", height: "300px" }} // Fixed height for each file container
        >
          <div className="bg-white rounded-md p-1 mb-1 w-full h-full flex flex-col justify-between">
            <p className="text-xs truncate w-full text-center bg-gray-300 p-1 rounded-md">
              {file.file_name}
            </p>
            {file.thumbnail_location ? ( // Check if thumbnail exists
              <img
                src={file.thumbnail_location}
                alt={file.file_name}
                className="w-full h-auto object-cover rounded-md mt-1"
              ...
              <div className="w-full h-auto bg-gray-300 rounded-md mt-1" /> // Placeholder if thumbnail doesn't exist
            )}
            <FaTrash
